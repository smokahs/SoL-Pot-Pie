...
@OnlyIn(Dist.CLIENT)
public class FoodQueueComponent implements ICustomComponent {
	int columns = 6;
	int rows = 6;
		...
		if (foods.isEmpty()) {
			if (showEmptyText) {
				graphics.drawString(Minecraft.getInstance().font,
						Component.literal(localized("gui", "food_book.queue.empty")).setStyle(context.getFont()),
						x, y, context.getTextColor(), false);
			}
			return;
		}
