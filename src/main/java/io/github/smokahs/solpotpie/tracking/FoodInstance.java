...
public final class FoodInstance {
	...
	public static FoodInstance decode(String encoded) {
		...
		if (item == null) {
			SOLPotPie.LOGGER.warn("attempting to load item into food list that is no longer registered: " + encoded + " (removing from list)");
			return null;
		}

		if (!item.isEdible()) {
			SOLPotPie.LOGGER.warn("attempting to load item into food list that is no longer edible: " + encoded + " (ignoring in case it becomes edible again later)");
		}
