package io.github.smokahs.solpotpie.command;
...
import io.github.smokahs.solpotpie.tracking.*;
...
public final class FoodListCommand {
	private static final String name = "solpotpie";
	...
	public static void register(RegisterCommandsEvent event) {
		event.getDispatcher().register(
			literal(name)
				...
				.then(withPlayerArgumentOrSender(literal("stats"), FoodListCommand::displayStats))
	...
	static int displayStats(CommandContext<CommandSourceStack> context, Player target) {
		...
		FoodList foodList = FoodList.get(target);
		double points = foodList.lifetimePoints();
	...
	static int clearFoodList(CommandContext<CommandSourceStack> context, Player target) {
		...
		HeartsHandler.updatePlayer(target);
		CapabilityHandler.syncFoodList(target);
